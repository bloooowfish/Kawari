...
use self::init::housing_furniture_objects_from_rows;
...
use crate::housing::scope::{
    housing_furniture_object_scope_for_estate, housing_furniture_object_scopes_for_estate,
};
#[cfg(test)]
...
impl ZoneConnection {
    ...
    pub async fn invalidate_housing_estate_from_admin(
        &mut self,
        land_ident: i64,
        ...
        let active_estate_matches = invalidate_housing_admin_cache(
            &mut self.active_housing_estate,
            &mut self.player_data.house_inventory,
            land_ident,
            clear_inventory,
        ...
        if clear_inventory {
            for scope in furniture_scopes {
                if should_replace_housing_furniture_scope_from_admin(
                    &scope,
                    ...
                    self.replace_housing_furniture_objects_from_admin(land_ident, scope)
                        .await;
                }
            }
        }

        if active_estate_matches {
            self.send_owned_housing().await;
        }
    }

    async fn replace_housing_furniture_objects_from_admin(
        &mut self,
        land_ident: i64,
        ...
        let furniture_objects = {
            ...
            database
                ...
                .filter(|query| {
                    housing_furniture_object_scopes_for_estate(&query.estate).contains(&scope)
                })
                .map(|query| {
        ...
        self.handle
            .send(ToServer::ReplaceHousingFurnitureObjects(
...
fn invalidate_housing_admin_cache(
    active_housing_estate: &mut Option<ActiveHousingEstate>,
    house_inventory: &mut HousingInventory,
    land_ident: i64,
    clear_inventory: bool,
    ...
    if !active_housing_estate
        .as_ref()
        .is_some_and(|estate| estate.land_ident == land_ident)
    {
        return false;
    }
...
fn should_replace_housing_furniture_scope_from_admin(
    scope: &HousingFurnitureObjectScope,
    ...
    if active_estate
        .is_some_and(|estate| estate.land_ident == land_ident && estate.indoors == scope.indoors)
    {
        return true;
    }
    ...
    !scope.indoors
        ...
        && zone_id == scope.territory_type_id
        && [display_context, active_context]
            .into_iter()
            .flatten()
            .any(|context| housing_ward_context_matches_scope(context, scope))
}

fn housing_ward_context_matches_scope(
    context: ActiveHousingWardContext,
    scope: &HousingFurnitureObjectScope,
) -> bool {
    context.territory_type_id == scope.territory_type_id && context.ward_index == scope.ward_index
}
...
mod tests {
    ...
    fn housing_furniture_object_from_row_rejects_rows_from_the_other_area() {
        ...
        assert!(housing_furniture_object_from_row(&exterior, false, 2, 5).is_some());
        assert!(housing_furniture_object_from_row(&exterior, true, 0, 0).is_none());
        ...
        assert!(housing_furniture_object_from_row(&interior, true, 0, 0).is_some());
        assert!(housing_furniture_object_from_row(&interior, false, 2, 5).is_none());
    }
    ...
    fn housing_furniture_objects_from_rows_keeps_only_matching_placed_area() {
        ...
        let indoor_objects = housing_furniture_objects_from_rows(&rows, true, 0, 0);
        let outdoor_objects = housing_furniture_objects_from_rows(&rows, false, 2, 5);
        ...
        assert_eq!(outdoor_objects[0].ward_index, 2);
    ...
    #[test]
    fn housing_admin_invalidation_clears_matching_inventory_without_active_selection() {
        let mut active_estate = Some(active_housing_estate(
            &HousingEstate {
                land_ident: 1005,
                house_id: house_id(5, 0, false).to_u64() as i64,
        ...
        *inventory
        ...
        assert!(invalidate_housing_admin_cache(
            &mut active_estate,
            &mut inventory,
            1005,
            true,
    ...
    #[test]
    fn housing_admin_invalidation_clears_matching_active_selection_when_requested() {
        let mut active_estate = Some(active_housing_estate(
            &HousingEstate {
                land_ident: 1005,
                house_id: house_id(5, 0, false).to_u64() as i64,
    ...
    #[test]
    fn housing_admin_invalidation_ignores_non_matching_active_selection() {
        let mut active_estate = Some(active_housing_estate(
            &HousingEstate {
                land_ident: 1005,
                house_id: house_id(5, 0, false).to_u64() as i64,
    ...
    fn admin_outdoor_scope() -> HousingFurnitureObjectScope {
    ...
    #[test]
    fn housing_admin_scope_replacement_allows_matching_outdoor_display_context() {
        assert!(should_replace_housing_furniture_scope_from_admin(
            &admin_outdoor_scope(),
            None,
            1005,
    ...
    #[test]
    fn housing_admin_scope_replacement_rejects_other_outdoor_ward_context() {
        assert!(!should_replace_housing_furniture_scope_from_admin(
            &admin_outdoor_scope(),
            None,
            1005,
    ...
    #[test]
    fn housing_admin_scope_replacement_allows_matching_active_indoor_estate() {
        let active_estate = ActiveHousingEstate {
            land_ident: 1005,
            house_id: house_id(5, 0, false),
            indoors: true,
        };
        let indoor_scope = HousingFurnitureObjectScope {
            territory_type_id: 340,
            world_id: 21,
            ward_index: 2,
            division: 0,
